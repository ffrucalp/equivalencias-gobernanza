...
<script>setTimeout(()=>{window.print();window.close()},500)<\/script>
  ...
  // ── Dashboard stats: agregar datos de TODAS las fuentes ──
  const dashboardStats = useMemo(() => {
    // 1. Desde reportes guardados (savedReports)
    let rTotal = 0, rParcial = 0, rSin = 0;
    const rUnis = new Set();
    savedReports.forEach(r => {
      rTotal   += (r.summary?.total || 0);
      rParcial += (r.summary?.parcial || 0);
      rSin     += (r.summary?.sin || 0);
      if (r.origin_university) rUnis.add(r.origin_university);
    });

    // 2. Desde tablas provisionales (savedTablas)
    let tTotal = 0, tParcial = 0, tSin = 0;
    savedTablas.forEach(t => {
      if (t.colors && typeof t.colors === "object") {
        Object.values(t.colors).forEach(v => {
          if (v === "TOTAL") tTotal++;
          else if (v === "PARCIAL") tParcial++;
          else if (v === "SIN_EQUIVALENCIA") tSin++;
        });
      }
      if (t.origin_university) rUnis.add(t.origin_university);
    });

    // 3. Desde análisis individuales legacy (analyses / localStorage)
    ...
    return {
      reportes: savedReports.length,
      tablas: savedTablas.length,
      planes: savedPlans.length,
      total:   rTotal + tTotal + aTotal,
      parcial: rParcial + tParcial + aParcial,
      sin:     rSin + tSin + aSin,
      universities: rUnis.size,
    };
  }, [savedReports, savedTablas, analyses, savedPlans]);

  const stats = { total: dashboardStats.total, parcial: dashboardStats.parcial, sin: dashboardStats.sin, universities: dashboardStats.universities };
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: C.text }}>
                          {tabla.origin_career || "Carrera no especificada"}
                        </div>
                        <div style={{ fontSize: 12, color: C.textMuted, marginTop: 3 }}>
                          {tabla.origin_university || "Universidad no especificada"} · {tTotal} materias evaluadas
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
